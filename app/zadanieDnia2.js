...
TEXT.split(' ').forEach( (el) => {
    code+= el[0]+el[el.length-1];
})

 const hashesFunction = ['aes192', 'aes-256-cbc', 'aes-256-ecb'];
hashesFunction.forEach((hashes)=>{
   try{ console.log(decodeText(ENCRYPTED_TEXT,code,hashes));}
   catch(err){
      console.log('zla funckja: '+ hashes);
   }
})
